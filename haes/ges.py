...
def main():
    ...
    for task in tasks:
        ...
        print(f"Fitting ges for task {task}, dataset {dataset}, fold {fold}")
        ges.fit(predictions_val, y_val)
        ...
        y_pred = ges.ensemble_predict_proba(predictions_test)
        if number_of_classes == 2:
            y_pred = y_pred[:, 1]
        ...
        print(f"\tFinal ensemble size: {len(ges.indices_)}")
        print(f"\tNumber of different base models in the ensemble: {len(set(ges.indices_))}")
